...
class TERRITORYBOSS_API ATBWeapon : public AActor
{
	GENERATED_BODY()
...
protected:
	...
	ATBWeapon(const FObjectInitializer& ObjectInitializer);
...
public:
	...
	void SetOwningPawn(ATBCharacter* NewOwner);

	/* Get pawn owner */
	UFUNCTION(BlueprintCallable, Category = "Weapon")
	class ATBCharacter* GetPawnOwner() const;

	virtual void OnEnterInventory(ATBCharacter* NewOwner);
	...
	UPROPERTY(EditDefaultsOnly, Category = "Weapon")
	TSubclassOf<class ATBWeaponPickup> WeaponPickupClass;
...
protected:
	...
	virtual void FireWeapon() PURE_VIRTUAL(ATBWeapon::FireWeapon, );
